...
class LIP(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, gt_transform=None ):
        ...
        if self.train:
            self.train_image_path, self.train_gt_path = self.read_labeled_image_list(osp.join(root))
        else:
            self.val_image_path, self.val_gt_path = self.read_labeled_image_list(osp.join(root))
    ...
    def read_labeled_image_list(self, data_dir):
        ...
        print(data_dir)
        # f = open(osp.join(data_dir, 'ImageSets','Segmentation','test.txt' ), 'r')
        f = open(osp.join(data_dir,'test.txt' ), 'r')
        image_path = []
        gt_path = []
        for line in f:
            image = line.strip("\n")
            if self.train:
                # image_path.append(osp.join(data_dir, 'JPEGImages', image + ".jpg"))
                # gt_path.append(osp.join(data_dir, 'SegmentationClass', image + ".png"))
                image_path.append(osp.join(data_dir, 'image', image + ".png"))
                gt_path.append(osp.join(data_dir, 'label', image + ".png"))
            else:
                image_path.append(osp.join(data_dir, 'image', image + ".png"))
                gt_path.append(osp.join(data_dir, 'label', image + ".png"))
...
class LIPWithClass(LIP):
    ...
    def __init__(self, root, num_cls=21, train=True, transform=None, gt_transform=None):
...
if __name__ == "__main__":
    ...
    path = 'C:\\Users\\93909\\Desktop\\NEW_pspnet\\train'
    ...
    # loader = data.DataLoader(LIPWithClass(path, transform=data_transforms['image'], gt_transform=data_transforms['gt']),
    #                          batch_size=1, shuffle=False)
